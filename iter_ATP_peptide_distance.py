...
for chunk in md.iterload('traj000000002.dcd', chunk=100, top=top):
    distance_list_1.append(md.compute_distances(chunk, phos_thr_new, periodic=True, opt=True))

distance_list_2 = []
for chunk in md.iterload('traj000000003.dcd', chunk=100, top=top):
    distance_list_2.append(md.compute_distances(chunk, phos_thr_new, periodic=True, opt=True))

# Convert lists to arrays
distance_array_1 = np.array(distance_list_1)
distance_array_new_1 = np.reshape(distance_array_1, (5000,1))

distance_array_2 = np.array(distance_list_2)
distance_array_new_2 = np.reshape(distance_array_2, (5000,1))

# Combine arrays
phos_thr_distance = np.concatenate((distance_array_new_1, distance_array_new_2), axis=0)
